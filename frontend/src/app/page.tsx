    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main id="main-content" className="flex-1">
        <section className="relative min-h-[90vh] flex items-center overflow-hidden pt-24">
          ...
          <div className="absolute bottom-40 -right-40 w-[500px] h-[500px] bg-cyan-500/10 rounded-full blur-3xl animate-pulse-glow" style={{ animationDelay: "1.5s" }} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-indigo-500/5 rounded-full blur-3xl" />

          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-5xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 border border-emerald-500/30 text-sm font-medium text-emerald-300 mb-8 animate-fade-in">
                <Sparkles className="h-3.5 w-3.5" />
                AI-powered career intelligence
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              </div>

              <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight leading-[0.95] mb-6 animate-slide-up">
                Land Your Dream Job
                <br />
                <span className="font-serif italic text-gradient">Faster</span> with AI
              </h1>
          ...
          <div className="container mx-auto px-4 relative">
            ...
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-5xl font-bold mb-4">
                Powerful <span className="text-gradient">resume tools</span>
              </h2>
              ...
              {features.map((f, i) => (
                <Link key={i} href={f.link}>
                  <Card className="group h-full cursor-pointer bg-transparent border border-border/50 hover:border-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-500 hover:-translate-y-1 overflow-hidden relative">
                    ...
                    <CardHeader>
                      <div className={`w-12 h-12 rounded-xl ${f.bg} flex items-center justify-center mb-3 transition-transform duration-500 group-hover:scale-110`}>
              ...
              {steps.map((s, i) => (
                <div key={i} className="text-center group">
                  <div className="relative mb-6 inline-block">
                    <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 border border-emerald-500/20 flex items-center justify-center mx-auto transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3">
                      <s.icon className="h-8 w-8 text-emerald-400" />
                    </div>
                    <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-gradient-brand text-white text-xs font-bold flex items-center justify-center glow-brand">
                      {s.step}
                    </div>
                  </div>
                ...
                <Card
                  ...
                  className={`relative overflow-hidden border ${
                    p.highlighted
                      ...
                      : "border-border/50"
                  } bg-transparent hover:border-emerald-500/30 transition-all duration-500`}
                >
                  {p.highlighted && (
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-brand" />
                  )}
                  {p.highlighted && (
                    <div className="absolute -top-10 -right-10 w-20 h-20 bg-gradient-brand/20 rounded-full blur-2xl" />
                  )}
                  <CardHeader>
